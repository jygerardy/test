...
function score() {
    ...
    checkInput(selectedVariable, featuresJson); //check if those three inputs are missing or not well formatted

    let counterFactuals = parseCounterFactuals(counterFactualsRaw, increment, isCategorical);
    let grid = generateGrid(selectedVariable, counterFactuals, featuresJson);


    graphContainer.innerHTML = '<div class="graph-placeholder">Scoring in progress. Wait...</div>';
    webappBackend.get('score', {grid: JSON.stringify(grid)})
...
function generateGrid(selectedVariable, counterFactuals, featuresJson) {
    if (isBlank(selectedVariable)) {
        let message = "Missing variable of interest. Enter the variable you would like to vary";
        displayAndThrowError(message, 'selected-feature');
    };
    let grid = {
        "items": []
    };
    for (let cf of counterFactuals) {
        let features = JSON.parse(featuresJson);
        features[selectedVariable] = cf;
        grid.items.push({features: features});
    }
    return grid
}
...
init();$.getJSON(getWebAppBackendUrl('/first_api_call'), function(data) {
